package com.example.androidbeats.demoapp;

/*

This module implements the bluetooth connectivity status functions such as connect,
disconnect, onconnected and onMessageSent,sendMessage, onMessageReceived, sendEmail functions
and monitor the temperature of the person and send the message on email if it's above
threshold value.
...
import com.example.androidbeats.BluetoothManager;
import com.example.androidbeats.SimpleBluetoothDeviceInterface;
...
//        messagesData.postValue(messages.toString());
        ...
        String[] words = myString.split("pp"); // Split the string into words using space as the delimiter
        String word1 = words[0]; // The first word
        String word2 = words[2];
        messages.append(word1).append('\n');
        messagesData.postValue(messages.toString());

//        int temp = Integer.parseInt(words[1]);
//        int beats = Integer.parseInt(words[2]);
        float num = Float.parseFloat(word2);
...
//              sendSms(PHONE_NUMBER);
    ...
    private void sendEmail() {
        SendMail mail = new SendMail("alerthm05@gmail.com", "healthmonitoring247",
                "engr.mohsinkhalid@gmail.com",
                "Alert Message",
                "Your contact temperature is above 98.");
        mail.execute();

        Toast.makeText(getApplication(), "sending email to your contact", Toast.LENGTH_SHORT).show();
    ...
    private void sendSms( String phoneNumber) {
        try {
            ...
            smsManager.sendTextMessage(phoneNumber, null, "Person Temperature is greater then 98", null, null);
            Toast.makeText(getApplication(), "SMS sent", Toast.LENGTH_SHORT).show();
        } catch (Exception e) {
            Toast.makeText(getApplication(), "SMS failed to send", Toast.LENGTH_SHORT).show();
            e.printStackTrace();
        }
    }
